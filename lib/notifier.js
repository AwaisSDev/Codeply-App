...
function showAliveNotification(uiRoot) {
  ...
  // OS-aware shortcut label: Cmd on macOS, Alt everywhere else — must match
  // the actual registered global hotkey (default 'Alt+C', see main.js).
  const shortcut = process.platform === 'darwin' ? 'Cmd + C' : 'Alt + C';
